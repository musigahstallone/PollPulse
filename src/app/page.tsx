    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto p-4 md:p-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-primary">Active & Past Elections</h1>
          <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
            Browse through the list of ongoing and completed elections. Cast your vote in active elections or view the results of past ones.
          </p>
        </div>

        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {elections.map((election) => {
            const isActive = election.startDate <= now && election.endDate >= now;
            return (
              <Card key={election.id} className="flex flex-col hover:shadow-xl transition-shadow duration-300">
                ...
                <CardContent className="flex-grow space-y-4">
                  <div className="flex items-center text-sm text-muted-foreground">
                    ...
                    <span>{format(election.startDate, 'PPP')} - {format(election.endDate, 'PPP')}</span>
                  </div>
                  <div className="flex items-center text-sm text-muted-foreground">
